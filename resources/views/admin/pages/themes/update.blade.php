...
@else
    ...
    <p>{{ count($blocksData) }} blocks found</p>
    ...
    <div class="table-responsive">
        <table id="themes-table" class="table table-striped table-bordered">
            ...
            @foreach($blocksData as $block => $blockData)
                <tr class="_{{ $block }} {{ ($blockData['run_template_update'])?'success':'' }}">
                    <td>{!! Form::checkbox('block['.$block.'][run_template_update]', 1, $blockData['run_template_update'], ['class' => 'form-control run-template-updates']) !!}</td>
                    ...
                    <td>{!! Form::checkbox('block['.$block.'][global_site]', 1, $blockData['global_site'], ['class' => 'form-control based-on-template-updates']) !!}</td>
                    <td>{!! Form::checkbox('block['.$block.'][global_pages]', 1, $blockData['global_pages'], ['class' => 'form-control based-on-template-updates']) !!}</td>
                </tr>
            @endforeach
            </tbody>
